...
(* Code 95% by Xavier Leroy with Scott additions labeled "SFS" *)
...
module Exception = struct
  ...
  let (>>=) = bind (* SFS: traditional infix version of bind function *)
  let ( let* ) = bind (* SFS: added in OCaml 4.08: let* macro built-in
...
(* SFS: 1+2 in the exception monad; change module name to run in other monads *)
(* Recall we are assuming code is in ANF so effectful program was 
   let onev = 1 in let twov = 2 in onev + twov *)

let oneplustwo = Exception.(bind (ret 1) (fun onev -> bind (ret 2) (fun twov -> ret (onev+twov))))

(* Equivalent version using >>= infix version of bind to look more like a let *) 
                   
let oneplustwo = Exception.((ret 1) >>= (fun onev -> (ret 2) >>= (fun twov -> ret (onev+twov))))

(* Even better, use the new let* macro which makes it look almost like regular code *)

let oneplustwo = Exception.(let* onev = ret 1 in let* twov = ret 2 in ret (onev+twov))

let runoneplustwo = Exception.run oneplustwo

(* SFS: an actual use of exceptions in exception monad *)

exception Error

(* SFS: OCaml code version of simple example Xavier had in slides *)

let fcaml x =
        try 1 + (if x = 0 then raise Error else 100 / x)
        with Error -> 101

(* Monad near-equivalent - note the + 1 moved in, bind needed if written exactly as above *)

let f x =
        Exception.(trywith (if x = 0 then raise Error else ret(1 + 100 / x))
                       (fun xn -> ret(101)))

(* While moving the 1+ in is the right thing to do for the above code, sometimes you can't..
   Lets as an exercise keep the 1 + in the original spot with bind (using new let* syntax)
   Recall we are coding "in ANF" when writing monadically *)
let f x =
        Exception.(trywith (let* one = ret 1 in 
                            let* ifthen = if x = 0 then raise Error else ret(100 / x) in
                              ret(one + ifthen))
                       (fun xn -> ret(101)))


(* The "100 / x" above is an example of smart escaping back to no-monad-land, 
     we could have stupidly written the equivalent in monad-land:
     let* hun = ret(100) in let* xx = ret(x) in ret(hun / x) 
     but don't - there are no side effects in 100 / x so short-circuit the monad. *)
...
(* SFS: Store can be heterogenous with OCaml GADTS*)
(* SFS: Example of the above GADT Store in action. *)

let (r0,s0) = Store.alloc "d" (Store.empty)
let (r1,s1) = Store.alloc 5 s0
let stringd = Store.read r0 s1
let num5 = Store.read r1 s1

(* Key in the above is refs like r0, r1 are not just numbers, they include inj/proj wrappers *)
...
module State = struct
  ...
  let (>>=) = bind
  let ( let* ) = bind (* SFS: lets use new OCaml "let*" *)
...
(* SFS: a "normal OCaml" stateful loop which we will re-code in the State monad *)
    
let sumlistnomonad l =
  let r = ref 0 in 
...
(* SFS: The above re-coded in the State monad. *)

let sumlist l =
  State.(run (let* r = ref 0 in
    let rec sum = function
    | [] -> deref r
    | hd :: tl -> let* n = deref r in 
                  let* _ = assign r (n + hd) in
                  sum tl
    in sum l))

(* Notice how the state operators themselves cannot have side effects in their arguments - 
   e.g. "ref (ref 5)" kind of thing cannot be written, need to ANF it.  
   Also observe we don't need to jump into monad for n + hd, there are no side effects there *)
...
let exple_cont n =
  Cont.(run 
    (callcc (fun k ->
       let* x = if n < 0 then throw k n else ret n in ret (x + 1))))


(* SFS: list predicate iterative example from slides in OCaml Cont monad.
   Need to re-code list_iter in the monad, List.iter is not "CPS-encoded" like we need.  
   Also the above Cont module expects the final result to be an int only so make "0" fake "None" *)

let rec list_iter cont_pred lst = 
Cont.(match lst with [] -> ret 0 | hd :: tl -> let* _ = cont_pred hd in list_iter cont_pred tl)

let find funcl_pred lst =
Cont.(callcc (fun k -> 
  let* _ = list_iter (fun x -> if funcl_pred x then throw k x else ret 0) lst in
    ret 0))

let testfind = Cont.run(find (fun x -> x>5)([1;4;2;56;44]))

(* the fancier backtracking example will need a better answer type in Cont .. following type errors now 
   and may in fact not be typable for any Cont since the answer itself includes a Cont - recursive *)
(* let backfind funcl_pred lst = 
Cont.(callcc (fun k ->
let* _ = list_iter 
           (fun x -> if funcl_pred x then callcc (fun k' -> throw k (Some(x, k'))) else ret None) lst 
           in ret None))

 -- illustrates the hazards of monad programming, types can get U-G-L-Y
*)
...
let log_abs n =
  Log.(run
    (if n >= 0 
     then let* _ = log "positive" in ret n
     else let* _ = log "negative" in ret (-n)))
